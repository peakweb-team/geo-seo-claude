---
name: geo-peakweb
description: Peakweb-customized GEO analysis tool with branded pitch deck generation and CLIENT-REPORT.md output. Use for Peakweb client deliverables.
...
|---------|-------------|
| `/geo-peakweb audit <url>` | Full GEO + SEO audit with parallel subagents |
| `/geo-peakweb page <url>` | Deep single-page GEO analysis |
| `/geo-peakweb citability <url>` | Score content for AI citation readiness |
| `/geo-peakweb crawlers <url>` | Check AI crawler access (robots.txt analysis) |
| `/geo-peakweb llmstxt <url>` | Analyze or generate llms.txt file |
| `/geo-peakweb brands <url>` | Scan brand mentions across AI-cited platforms |
| `/geo-peakweb platforms <url>` | Platform-specific optimization (ChatGPT, Perplexity, Google AIO) |
| `/geo-peakweb schema <url>` | Detect, validate, and generate structured data |
| `/geo-peakweb technical <url>` | Traditional technical SEO audit |
| `/geo-peakweb content <url>` | Content quality and E-E-A-T assessment |
| `/geo-peakweb report <url>` | Generate client-ready GEO deliverable |
| `/geo-peakweb report-pdf <url>` | Generate professional PDF report with charts and scores |
| `/geo-peakweb quick <url>` | 60-second GEO visibility snapshot |
...
### Full Audit (`/geo-peakweb audit <url>`)
...
| Command | Output File |
|---------|------------|
| `/geo-peakweb audit` | `GEO-AUDIT-REPORT.md` |
| `/geo-peakweb page` | `GEO-PAGE-ANALYSIS.md` |
| `/geo-peakweb citability` | `GEO-CITABILITY-SCORE.md` |
| `/geo-peakweb crawlers` | `GEO-CRAWLER-ACCESS.md` |
| `/geo-peakweb llmstxt` | `llms.txt` (ready to deploy) |
| `/geo-peakweb brands` | `GEO-BRAND-MENTIONS.md` |
| `/geo-peakweb platforms` | `GEO-PLATFORM-OPTIMIZATION.md` |
| `/geo-peakweb schema` | `GEO-SCHEMA-REPORT.md` + generated JSON-LD |
| `/geo-peakweb technical` | `GEO-TECHNICAL-AUDIT.md` |
| `/geo-peakweb content` | `GEO-CONTENT-ANALYSIS.md` |
| `/geo-peakweb report` | `GEO-CLIENT-REPORT.md` (presentation-ready) |
| `/geo-peakweb report-pdf` | `GEO-REPORT.pdf` (professional PDF with charts) |
| `/geo-peakweb quick` | Inline summary (no file) |
...
The `/geo-peakweb report-pdf <url>` command generates a professional, branded PDF report:
...
### Workflow
1. First run `/geo-peakweb audit <url>` to collect all data
2. Then run `/geo-peakweb report-pdf <url>` to generate the PDF
